...
The full procedure — the split-tree layout, what survives a deploy, the backup, the upload,
permissions, verification and rollback — lives in **[deployment.md](deployment.md)**.

Two facts are worth repeating here, because ignoring either one causes real damage:

- **Two trees, two destinations.** `blwp/api/` → the document root; `blwp/backend/` →
  `/home/deploy/blwp/` with the `backend/` level dropped. `bootstrap.php` picks the
  production branch by testing for `/home/deploy/blwp/config/api-config.php`; if that file is
  missing or unreadable it **silently** falls back to development paths.
- **Four paths are runtime state and must never be uploaded**: `config/secrets.php`,
  `config/site-mapping.json`, `config/cron-state.json`, `config/alerts.json`.

> These steps used to be duplicated here. They now live in one place only —
> [deployment.md](deployment.md). Any other copy is stale.
